...
function loadHomepageIndex() {
    ...
    fetch("content/index.json")
        ...
        .then(data => {
            ...
            if (hp.jammuKashmir) {
                if (hp.jammuKashmir[0]) loadArticleToCard(hp.jammuKashmir[0], "jk1-media", "jk1-body");
                if (hp.jammuKashmir[1]) loadArticleToCard(hp.jammuKashmir[1], "jk2-media", "jk2-body");
            }

            if (hp.international) {
                if (hp.international[0]) loadArticleToCard(hp.international[0], "intl1-media", "intl1-body");
                if (hp.international[1]) loadArticleToCard(hp.international[1], "intl2-media", "intl2-body");
            }

            if (hp.humanRights) {
                if (hp.humanRights[0]) loadArticleToCard(hp.humanRights[0], "hr1-media", "hr1-body");
                if (hp.humanRights[1]) loadArticleToCard(hp.humanRights[1], "hr2-media", "hr2-body");
            }
...
function initTicker() {
    ...
    fetch("content/index.json")
        ...
        .then(data => {
            ...
            ul.classList.add("ticker-scroll");
            ...
            grid.innerHTML = data.videos.map(v => `
                <article class="card">
                    <div class="vlog-thumb">
                        <iframe src="https://www.youtube.com/embed/${v.youtubeId}" allowfullscreen></iframe>
                    </div>
                    <div class="card-body">
                        <h3>${v.title}</h3>
                        <p>${v.description}</p>
...
function renderFullArticlePage(article) {
    ...
    if (sectionLabel) sectionLabel.textContent = article.category || "";
